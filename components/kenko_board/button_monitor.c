...
static void button_task(void *arg)
{
    ...
    for (;;) {
        bool sample = gpio_get_level(CONFIG_KENKO_BUTTON_GPIO) == 0;
        ...
        if (stable_pressed) {
            pressed_ms += CONFIG_KENKO_BUTTON_POLL_INTERVAL_MS;

            if (!provisioning_sent && pressed_ms >= CONFIG_KENKO_BUTTON_PROVISIONING_MS) {
                ESP_LOGW(TAG, "long press %u ms: entering provisioning mode",
                         (unsigned)CONFIG_KENKO_BUTTON_PROVISIONING_MS);
                kenko_event_post(KENKO_EVENT_ENTER_PROVISIONING);
                provisioning_sent = true;
            }

            if (!factory_reset_sent && pressed_ms >= CONFIG_KENKO_BUTTON_FACTORY_RESET_MS) {
                ESP_LOGW(TAG, "long press %u ms: factory reset",
                         (unsigned)CONFIG_KENKO_BUTTON_FACTORY_RESET_MS);
                kenko_event_post(KENKO_EVENT_FACTORY_RESET);
                factory_reset_sent = true;
            }
        }

        vTaskDelay(pdMS_TO_TICKS(CONFIG_KENKO_BUTTON_POLL_INTERVAL_MS));
    }
}
...
esp_err_t button_monitor_start(void)
{
    const gpio_config_t config = {
        .pin_bit_mask = 1ULL << CONFIG_KENKO_BUTTON_GPIO,
    ...
    BaseType_t created = xTaskCreate(button_task, "button_monitor", CONFIG_KENKO_BUTTON_TASK_STACK, NULL,
                                     CONFIG_KENKO_BUTTON_TASK_PRIORITY, NULL);
