...
esp_err_t status_led_init(void)
{
    ...
    const led_strip_config_t strip_config = {
        .strip_gpio_num = CONFIG_KENKO_WS2812_GPIO,
        .max_leds = 1,
    ...
    s_led.brightness = CONFIG_KENKO_LED_DEFAULT_BRIGHTNESS;

    BaseType_t created = xTaskCreate(led_task, "status_led", CONFIG_KENKO_LED_TASK_STACK, NULL,
                                     CONFIG_KENKO_LED_TASK_PRIORITY, NULL);
    ESP_RETURN_ON_FALSE(created == pdPASS, ESP_ERR_NO_MEM, TAG, "led task create failed");

    ESP_LOGI(TAG, "status LED initialized on GPIO %d", CONFIG_KENKO_WS2812_GPIO);
    return ESP_OK;
}
