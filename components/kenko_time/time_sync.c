...
void time_sync_apply_timezone(const char *timezone)
{
    if (timezone == NULL || timezone[0] == '\0') {
        timezone = CONFIG_KENKO_DEFAULT_TIMEZONE;
    }
...
esp_err_t time_sync_start(void)
{
    ...
    esp_sntp_config_t config = ESP_NETIF_SNTP_DEFAULT_CONFIG_MULTIPLE(
        2, ESP_SNTP_SERVER_LIST(CONFIG_KENKO_SNTP_SERVER_PRIMARY, CONFIG_KENKO_SNTP_SERVER_SECONDARY));
    config.start = true;
    ...
    s_sntp_running = true;
    ESP_LOGI(TAG, "sntp started (%s, %s)", CONFIG_KENKO_SNTP_SERVER_PRIMARY,
             CONFIG_KENKO_SNTP_SERVER_SECONDARY);
    return ESP_OK;
}
