...
static void dns_task(void *arg)
{
    ...
    inet_aton(CONFIG_KENKO_AP_IP, &answer_addr);
    ...
    while (atomic_load(&s_running)) {
        ...
        size_t response_len =
            dns_message_build_response(request, (size_t)received, answer_addr.s_addr,
                                       CONFIG_KENKO_DNS_TTL_SECONDS, response, sizeof(response));
        if (response_len > 0) {
...
esp_err_t dns_captive_start(void)
{
    ...
    const struct sockaddr_in addr = {
        .sin_family = AF_INET,
        .sin_port = htons(CONFIG_KENKO_DNS_PORT),
    ...
    BaseType_t created = xTaskCreate(dns_task, "dns_captive", CONFIG_KENKO_DNS_TASK_STACK, NULL,
                                     CONFIG_KENKO_DNS_TASK_PRIORITY, NULL);
    if (created != pdPASS) {
        atomic_store(&s_running, false);
        close(s_socket);
        s_socket = -1;
    ...
    ESP_LOGI(TAG, "dns captive server started on %s:%d", CONFIG_KENKO_AP_IP, CONFIG_KENKO_DNS_PORT);
    return ESP_OK;
}
