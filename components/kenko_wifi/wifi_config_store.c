...
#define WIFI_CONFIG_FILE CONFIG_KENKO_STORAGE_BASE_PATH "/" CONFIG_KENKO_WIFI_CONFIG_FILE_NAME
...
static esp_err_t persist_locked(void)
{
    ...
    esp_err_t err = json_file_write(WIFI_CONFIG_FILE, root);
    cJSON_Delete(root);
    return err;
}
...
esp_err_t wifi_config_store_init(void)
{
    ...
    cJSON *root = NULL;
    esp_err_t err = json_file_read(WIFI_CONFIG_FILE, &root);
