...
#if CONFIG_KENKO_WIFI_PS_NONE
#define KENKO_WIFI_PS_MODE WIFI_PS_NONE
#elif CONFIG_KENKO_WIFI_PS_MAX_MODEM
#define KENKO_WIFI_PS_MODE WIFI_PS_MAX_MODEM
#else
#define KENKO_WIFI_PS_MODE WIFI_PS_MIN_MODEM
#endif
...
/* 这三个标志被 WiFi 事件回调、连接循环任务和 HTTP 任务并发读写。
 * volatile 在 C 里既不保证原子性也不建立 happens-before，正确的原语是 _Atomic。 */
static atomic_bool s_link_up; /* 已拿到 IP */
static atomic_bool s_ap_active;
static atomic_bool s_connecting;

static wifi_scan_entry_t s_scan_cache[CONFIG_KENKO_WIFI_SCAN_MAX_RESULTS];
...
static void wifi_event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    ...
    if (event_base == WIFI_EVENT) {
        ...
        case WIFI_EVENT_STA_DISCONNECTED: {
            bool was_up = atomic_load(&s_link_up);
            ...
            if (was_up) {
                ...
                kenko_event_post(KENKO_EVENT_WIFI_LOST);
            } else {
    ...
    if (event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP) {
        ...
        kenko_event_post(KENKO_EVENT_WIFI_CONNECTED);
    }
}
...
static esp_err_t configure_softap_network(void)
{
    ...
    ESP_RETURN_ON_ERROR(esp_netif_str_to_ip4(CONFIG_KENKO_AP_IP, &ip_info.ip), TAG,
                        "CONFIG_KENKO_AP_IP is not a valid IPv4 address");
    ESP_RETURN_ON_ERROR(esp_netif_str_to_ip4(CONFIG_KENKO_AP_NETMASK, &ip_info.netmask), TAG,
                        "CONFIG_KENKO_AP_NETMASK is not a valid IPv4 mask");
...
static bool try_connect(const wifi_credential_t *credential)
{
    ...
    EventBits_t bits =
        xEventGroupWaitBits(s_events, BIT_STA_GOT_IP | BIT_STA_ATTEMPT_FAILED | BIT_LOOP_STOP_REQUEST,
                            pdFALSE, pdFALSE, pdMS_TO_TICKS(CONFIG_KENKO_WIFI_CONNECT_TIMEOUT_MS));
...
static void sta_connect_task(void *arg)
{
    ...
    while (!stop_requested()) {
        ...
        ESP_LOGW(TAG, "all %u wifi configs failed, retrying in %u ms", (unsigned)list.count,
                 (unsigned)CONFIG_KENKO_WIFI_RETRY_BACKOFF_MS);
        if (!interruptible_delay(CONFIG_KENKO_WIFI_RETRY_BACKOFF_MS)) {
            break;
        }
    }
...
esp_err_t wifi_manager_init(const char *sta_hostname)
{
    ...
    /* 默认事件循环由应用层在更早的位置创建；这里容忍"已存在"，
     * 让本组件既能被 app_main 驱动，也能被别的项目单独使用。 */
    esp_err_t loop_err = esp_event_loop_create_default();
    if (loop_err != ESP_OK && loop_err != ESP_ERR_INVALID_STATE) {
        ESP_RETURN_ON_ERROR(loop_err, TAG, "event loop create failed");
    }
    ...
    /* DHCP 上报的主机名必须是合法的 DNS 标签，所以由调用方传入 MAC 派生的固定名，
     * 而不是用户可以随便填的设备名。设置失败也不该拖垮启动。 */
    if (sta_hostname != NULL && sta_hostname[0] != '\0') {
        esp_err_t hostname_err = esp_netif_set_hostname(s_sta_netif, sta_hostname);
    ...
    /* 显式声明省电策略，而不是依赖默认值：MIN_MODEM 会带来最多一个 DTIM 周期的延迟抖动。 */
    ESP_RETURN_ON_ERROR(esp_wifi_set_ps(KENKO_WIFI_PS_MODE), TAG, "set power save mode failed");
    ESP_LOGI(TAG, "wifi started, power save mode=%d", (int)KENKO_WIFI_PS_MODE);
    return ESP_OK;
}
...
esp_err_t wifi_manager_start_sta_loop(void)
{
    ...
    BaseType_t created = xTaskCreate(sta_connect_task, "wifi_sta_loop", CONFIG_KENKO_WIFI_TASK_STACK, NULL,
                                     CONFIG_KENKO_WIFI_TASK_PRIORITY, &s_sta_task);
...
esp_err_t wifi_manager_start_provisioning_ap(void)
{
    ...
    config.ap.max_connection = CONFIG_KENKO_AP_MAX_CONNECTION;
...
void wifi_manager_get_status(wifi_status_t *out)
{
    ...
    out->sta_connected = atomic_load(&s_link_up);
    out->ap_active = atomic_load(&s_ap_active);
    out->connecting = atomic_load(&s_connecting) && !atomic_load(&s_link_up);
...
static void collect_unique(const wifi_ap_record_t *records, uint16_t record_count)
{
    ...
    for (uint16_t index = 0; index < record_count; ++index) {
        ...
        if (merged || s_scan_cache_count >= CONFIG_KENKO_WIFI_SCAN_MAX_RESULTS) {
            continue;
        }
...
static esp_err_t refresh_scan_cache(void)
{
    ...
    if (ap_count > CONFIG_KENKO_WIFI_SCAN_MAX_RESULTS) {
        ap_count = CONFIG_KENKO_WIFI_SCAN_MAX_RESULTS;
    }
...
esp_err_t wifi_manager_scan(wifi_scan_entry_t *out, size_t max, size_t *count, bool force)
{
    ...
    *count = 0;
    ...
    bool cache_valid = s_scan_cache_at_us >= 0 &&
                       (now - s_scan_cache_at_us) < (int64_t)CONFIG_KENKO_WIFI_SCAN_CACHE_TTL_MS * 1000;
