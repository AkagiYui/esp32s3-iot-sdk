...
{
    /* 配置分区可以随时重建，挂不上就格式化。 */
    s_storage_available = mount_partition(KENKO_STORAGE_PARTITION, KENKO_STORAGE_BASE_PATH, true);

    /* 前端资源分区由构建产物写入，不能随便格式化，否则会把页面抹掉。 */
    s_web_available = mount_partition(KENKO_WEB_PARTITION, KENKO_WEB_BASE_PATH, false);
...
{
    ESP_LOGW(TAG, "formatting %s partition", KENKO_STORAGE_PARTITION);
    esp_err_t err = esp_littlefs_format(KENKO_STORAGE_PARTITION);
    if (err != ESP_OK) {
